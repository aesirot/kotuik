    ...
    public <T> @NotNull byte[] serialize(final @NotNull @NonNull T t) {
        ...
        try {
            ...
            if (t instanceof QluaEvent.EventType) {
                ...
                String strResult = objectMapper.writeValueAsString(t); //ANTON (не пригодилось)
                result = strResult.getBytes(Charset.forName("CP1251")); //ANTON (не пригодилось)
            }
    ...
    public <T> @NotNull T deserialize(final @NotNull Class<T> clazz, final @NotNull byte[] data) {
        ...
        try {
            ...
            if (QluaEvent.EventType.class == clazz) {
                ...
                return objectMapper.readValue(new String(data, Charset.forName("CP1251")), clazz);
                //ANTON return objectMapper.readValue(data, clazz);
            }
        } catch (final Exception ex) {
            throw new SerdeException(
